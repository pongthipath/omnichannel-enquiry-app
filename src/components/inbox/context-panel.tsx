...
import { eventText } from '../../helpers/message.helper';
...
import { Enquiry, ENQUIRY_TYPES, PRIORITIES } from '../../services/enquiry.service';
import colors from '../../theme/colors';
import { cn } from '../../utils/cn';
import { Alert, Avatar, Badge, Button, Icon, InfoRow, SectionLabel, Select, Spinner, UnderlineTabs } from '../common';
...
type Tab = 'customer' | 'detail' | 'history';

/** Right column (design Main.dc.html): tabs for the open enquiry only — customer, details, history. */
export function ContextPanel({ enquiryId, onOpenEnquiry }: { enquiryId?: string; onOpenEnquiry: (id: string) => void }) {
    ...
    <>
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1">
          <Text className="font-bold text-[15px] text-dark dark:text-white">{enquiry.customer?.companyName}</Text>
          <Text className="font-sans text-[13px] text-body dark:text-body-dark">
...
function DetailTab({ enquiry: e }: { enquiry: Enquiry }) {
  ...
  return (
    <>
      ...
      <InfoRow label={t('inbox.panel.created')} value={formatDateTime(e.createdAt, 'th')} />
    ...
    <View className="gap-3.5">
      {events.map((m) => (
        <View key={m.id} className="flex-row gap-2.5">
          ...
          <View className="flex-1">
            <Text className="font-semibold text-[13px] text-dark dark:text-white">{eventText(m, t)}</Text>
            <Text className="font-sans text-xs text-body">{formatDateTime(m.createdAt, i18n.language)}</Text>
          </View>
        </View>
      ))}
